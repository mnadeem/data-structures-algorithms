...
public class LevenshteinWordDistanceCalculator {
    ...
    public int calculate(CharSequence source, CharSequence target) {
        ...
        // source prefixes can be transformed into empty string by dropping all characters (A move straight down/up is a deletion)
        // i.e If target string is empty, only option is to remove all characters of target string
        for (int row = 1; row <= sourceLength; row++) {
            grid[row][0] = row;
        }
        
        // target prefixes can be reached from empty source prefix by inserting every character (A move to the right/left is an insertion;)
        // i.e. If source string is empty, only option is to insert all characters of target string
